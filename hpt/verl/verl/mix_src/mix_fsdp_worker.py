...
class MIXActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_off_sequences(self, prompts: DataProto):
        # 确保这个方法存在于 MIXActorRolloutRefWorker 中
        prompts = prompts.to('cuda')
        assert self._is_rollout
        ...
        with self.rollout_sharding_manager:
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            output = self.rollout.generate_off_sequences(prompts=prompts)
            output = self.rollout_sharding_manager.postprocess_data(output)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_on_sequences(self, prompts: DataProto, on_num: int = None):
        # 确保这个方法存在于 MIXActorRolloutRefWorker 中
        prompts = prompts.to('cuda')
        assert self._is_rollout
        ...
        with self.rollout_sharding_manager:
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            output = self.rollout.generate_on_sequences(prompts=prompts, on_num=on_num)
            output = self.rollout_sharding_manager.postprocess_data(output)
