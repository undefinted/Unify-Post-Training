...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, grpo_use_std=True):
...
class RayPPOTrainer(object):
    ...
    def init_workers(self):
        ...
        if self.config.algorithm.adv_estimator == 'gae':
            ...
            self.use_critic = False
        elif self.config.algorithm.adv_estimator == 'grpo_surprisal':
            self.use_critic = False
    ...
    def _save_checkpoint(self):
        ...
        # save sampler state
        torch.save(self.train_dataloader.sampler.get_state(), os.path.join(local_global_step_folder, 'sampler_state.pt'), pickle_module=dill)
        ...
        try:
            ...
            for each_save_step in prev_save_steps:
                ...
                if not os.path.exists(prev_save_path):
                    continue
                # if exists, remove all optim.pt & model.pt
                for path in os.listdir(prev_save_path):
                    ...
                    if path.startswith('model_world_size') and path.endswith('.pt'):
                        os.remove(os.path.join(prev_save_path, path))
                        print(f'Removed {os.path.join(prev_save_path, path)}')
