...
def make_openr1_map_fn(split: str):
    ...
    def process_fn(example: Dict[str, Any], idx: int) -> Optional[Dict[str, Any]]:
        ...
        if 'is_reasoning_complete' in example: # open-r1-data
            assert split == 'train'
...
def filter_function(example):
    ...
    for i in range(len(correctness_math_verify)):
        if correctness_math_verify[i] == True and is_reasoning_complete[i] == True:
            gens.append(example['generations'][i])
    example['generations'] = gens
    return example
...
if __name__ == '__main__':
    ...
    def filter_target_length(example):
        gens = []
        for i, generation in enumerate(example['generations']):
            generation_input_ids = tokenizer(generation, add_special_tokens=False)['input_ids']
            if len(generation_input_ids) > 8192:
                pass
            else:
